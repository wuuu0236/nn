...
class PathPlanner:
    ...
    def get_exploration_target(self, current_pos, safe_directions):
        ...
        # 随机选择一个安全方向
        direction = safe_directions[np.random.randint(len(safe_directions))]['direction']
        # 向前飞行4米
        target = current_pos + direction * 4.0
        target[2] = 2.0  # 保持高度
...
class QuadrotorSimulation:
    ...
    def calculate_safe_movement(self, current_pos, target_pos):
        ...
        if safety_factor < 0.3:
            ...
            if np.linalg.norm(avoidance_force) > 0:
                ...
                print(f"⚠️ 紧急避障！安全系数: {safety_factor:.2f}")
        ...
        # 计算速度
        target_velocity = move_dir * self.max_speed * (0.8 + safety_factor * 0.4)
    ...
    def get_next_target(self, current_pos):
        ...
        if self.flight_phase == "takeoff":
            ...
            # 巡航阶段：使用路径规划器
            target = self.path_planner.get_next_waypoint(current_pos)
    ...
    def simulation_loop(self, viewer, duration):
        ...
        while (viewer is None or (viewer and viewer.is_running())) and (time.time() - start_time) < duration:
            ...
            if time.time() - last_print_time > 1.0:
                ...
                print(f"目标: 航点 {self.path_planner.current_waypoint + 1}")
...
def main():
    ...
    try:
        ...
        # ========== 可调参数 ==========
        sim.max_speed = 2.2  # 最大速度
        sim.takeoff_height = 2.0  # 起飞高度
        sim.cruise_height = 2.0  # 巡航高度

        # 感知系统参数
        sim.sensor_system.lidar_range = 6.0  # 增大探测范围
