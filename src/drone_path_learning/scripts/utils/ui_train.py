...
class TrainingUi(QWidget):
    ...
    def init_ui(self):
        ...
        pg.setConfigOption("imageAxisOrder", "row-major")  # 最佳性能
        ...
        self.pen_red = pg.mkPen(color="r", width=2)  # 用于命令曲线
        self.pen_blue = pg.mkPen(color="b", width=1)  # 用于真实数据
        self.pen_green = pg.mkPen(color="g", width=2)

    def update_value_list(self, list, value):
        """
        @description: 更新绘图用数值列表
        @param {type}
        @return:
        """
        list[:-1] = list[1:]
    ...
    def create_actionPlot_groupBox_multirotor(self):
        ...
        self.plotWidget_yaw_rate.setYRange(
            max=self.cfg.getfloat("multirotor", "yaw_rate_max_deg"),
